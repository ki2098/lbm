...
const double Tutbine_y = Turbine_y_/Cx_;
...
double9_t pdf_to_cumulant(
    ...
    central_to_cumulant(raw_to_central(pdf_to_raw(f), U), U);
}
...
double9_t cumulant_to_pdf(
    ...
    raw_to_pdf(central_to_raw(cumulant_to_central(c, shift), U));
}
...
void compute_cumulant(
    ...
    present(f[:sz[0]*sz[1]], c[:sz[0]*sz[1]], Ve, Cmk) \
    firstprivate(sz)
...
void compute_post_pdfs(
    ...
    #pragma acc parallel loop independent \
    present(cp[:sz[0]*sz[1]], shift[:sz[0]*sz[1]], fpost[:sz[0]*sz[1]]) \
    firstprivate(sz)
    for (int i = 0; i < sz[0]*sz[1]; i ++) {
        fpost[i] = cumulant_to_pdf(cp[i], shift[i]);
    }
}
...
void do_streaming(
    ...
    present(fpost[:sz[0]*sz[1]], f[:sz[0]*sz[1]]) \
    firstprivate(sz)
...
void apply_bc(
    ...
    for (int j = 1; j < sz[1] - 1; j ++) {
        ...
        get_macroscopic_val(f[l0], shift[l0], U0, density0);
        get_macroscopic_val(f[l1], shift[l1], U1, density1);
        get_macroscopic_val(f[l2], shift[l2], U2, density2);
        double2_t gradient{{
